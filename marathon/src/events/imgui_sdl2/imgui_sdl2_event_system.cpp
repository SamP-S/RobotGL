...
ImGuiSDL2EventSystem::ImGuiSDL2EventSystem()
    : IEventSystem("marathon.events.imgui_sdl2.imgui_sdl2_event_system") {}

ImGuiSDL2EventSystem::~ImGuiSDL2EventSystem() {}

bool ImGuiSDL2EventSystem::Init() {
    if (SDL_InitSubSystem(SDL_INIT_EVENTS) != 0) {
        MT_CORE_ERROR("events/imgui_sdl2/imgui_sdl2_event_system.cpp: SDL_Init Error = {}", SDL_GetError());
        return false;
    }
    return true;
}
void ImGuiSDL2EventSystem::Quit() {
 ...
 */
bool ImGuiSDL2EventSystem::PollEvent(Event& e) {
    SDL_Event sdl_e;
    int pending = SDL_PollEvent(&sdl_e);
    ImGui_ImplSDL2_ProcessEvent(&sdl_e);
    Convert(sdl_e, e);
    return pending;
}
 ...
 */
bool ImGuiSDL2EventSystem::WaitEvent(Event& e) {
    SDL_Event sdl_e;
...
bool ImGuiSDL2EventSystem::Convert(const SDL_Event& sdl_e, Event& e) {
    switch (sdl_e.type) {
        case SDL_QUIT:
...
bool ImGuiSDL2EventSystem::ConvertWindowEvent(const SDL_Event& sdl_e, Event& e) {
...
bool ImGuiSDL2EventSystem::ConvertMouseEvent(const SDL_Event& sdl_e, Event& e) {
    std::unordered_map<std::string, EventProperty> data;
    switch (sdl_e.type) {
        case SDL_MOUSEMOTION:
...
bool ImGuiSDL2EventSystem::ConvertKeyboardEvent(const SDL_Event& sdl_e, Event& e) {
    std::unordered_map<std::string, EventProperty> data;
    switch (sdl_e.type) {
        case SDL_KEYDOWN:
